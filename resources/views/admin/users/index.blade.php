<x-app-layout>
    ...
    <div class="py-12" x-data="{ showBulkModal: false, verificationText: '', totalRecords: {{ $users->total() }} }">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 sticky top-4">
                        ...
                        <form action="{{ route('admin.users.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <x-input type="text" name="name" class="mt-1 block w-full" required />
                                @error('name') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700">Email</label>
                                <x-input type="email" name="email" class="mt-1 block w-full" required />
                                @error('email') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-3">
                                <label class="block text-sm font-medium text-gray-700">Role</label>
                                <select name="role" class="mt-1 block w-full rounded-lg shadow-sm border-gray-300 text-gray-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 py-2 px-3 leading-tight transition duration-150 ease-in-out">
                            ...
                            <div class="mb-3">
                                ...
                                <x-input type="password" name="password" class="mt-1 block w-full" required />
                                @error('password') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div class="mb-4">
                                <label class="block text-sm font-medium text-gray-700">Konfirmasi Password</label>
                                <x-input type="password" name="password_confirmation" class="mt-1 block w-full" required />
                            </div>
                ...
                <div class="md:col-span-2">
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6 bg-white border-b border-gray-200">
                            ...
                            <!-- Filter & Search Form -->
                            <div class="mb-6">
                                <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col md:flex-row justify-between items-end gap-4 flex-wrap">
                                    <div class="flex flex-col md:flex-row gap-2 w-full md:w-auto flex-grow">
                                        <div class="w-full md:w-auto">
                                            ...
                                            <select name="role" id="role" class="rounded-lg shadow-sm border-gray-300 text-gray-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 py-2 px-3 text-sm leading-tight transition duration-150 ease-in-out w-full">
                                        ...
                                        <div class="w-full md:w-auto">
                                            <label for="start_date" class="block text-xs font-medium text-gray-500 mb-1">Tanggal Daftar</label>
                                            <x-input type="date" name="start_date" id="start_date" value="{{ request('start_date') }}" class="text-sm w-full" />
                                        </div>
                                        <div class="w-full md:w-auto">
                                            <label for="end_date" class="block text-xs font-medium text-gray-500 mb-1">s/d</label>
                                            <x-input type="date" name="end_date" id="end_date" value="{{ request('end_date') }}" class="text-sm w-full" />
                                        </div>
                                    </div>
                                    
                                    <div class="flex items-end gap-2 w-full md:w-auto justify-end">
                                        ...
                                        @if(request()->hasAny(['role', 'search', 'start_date', 'end_date']))
                                            <a href="{{ route('admin.users.index') }}" class="inline-flex items-center px-3 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:text-gray-800 active:bg-gray-50 transition ease-in-out duration-150 h-10">
                                                Reset
                                            </a>
                                            
                                            <!-- Tombol Bulk Delete (Hanya muncul jika ada filter) -->
                                            <button type="button" @click="showBulkModal = true" class="inline-flex items-center px-3 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150 h-10 ml-2 shadow-md">
                                                Bulk Delete
                                            </button>
                                        @endif
                                    </div>
