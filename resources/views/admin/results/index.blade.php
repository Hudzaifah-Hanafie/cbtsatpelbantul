<x-app-layout>
    ...
    <div class="py-12" x-data="{ showBulkModal: false, verificationText: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6">
                        <form method="GET" action="{{ route('admin.results.index') }}" class="flex flex-col md:flex-row justify-between items-end gap-4 flex-wrap">
                            
                            <div class="flex flex-col md:flex-row gap-4 w-full md:w-auto flex-grow">
                                <!-- Search -->
                                <div class="w-full md:w-1/4">
                                    <label for="search" class="block text-sm font-medium text-gray-700">Cari Peserta (Nama/Email)</label>
                                    <x-input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Cari..." class="mt-1 block w-full" />
                                </div>

                                <!-- Filter Test -->
                                <div class="w-full md:w-1/4">
                                    <label for="test_id" class="block text-sm font-medium text-gray-700">Filter Ujian</label>
                                    <select name="test_id" id="test_id" class="mt-1 block w-full rounded-lg shadow-sm border-gray-300 text-gray-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 py-2 px-3 leading-tight transition duration-150 ease-in-out">
                            ...
                            <div class="flex items-end gap-2 w-full md:w-auto justify-end">
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 h-10 shadow-sm">
                                    Filter
                                </button>
                                @if(request()->hasAny(['search', 'test_id', 'start_date', 'end_date', 'sort']))
                                    ...
                                    <button type="button" @click="showBulkModal = true" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150 h-10 ml-2 shadow-md">
                                        Bulk Delete
                                    </button>
                                @endif
